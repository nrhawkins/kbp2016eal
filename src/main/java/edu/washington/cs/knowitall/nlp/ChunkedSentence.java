...
public class ChunkedSentence extends BIOLayeredSequence {
	...
	/**
	 * The layer name for the Basic Normalized tokens.
	 */
	public static final String NORM_LAYER = "norm";
	...
	public ChunkedSentence(List<Range> offsets, List<String> tokens, List<String> posTags,
			List<String> npChunkTags) throws SequenceException {
		this(offsets == null ? null : ImmutableList.copyOf(offsets), ImmutableList.copyOf(tokens), ImmutableList
				.copyOf(posTags),
				ImmutableList.copyOf(npChunkTags));
	}
	...
	/**
	 * @return an unmodifiable list over the ranges of the VP chunks in this
	 *         sentence.
	 */
	public ImmutableCollection<Range> getVpChunkRanges() {
		return getSpans(NP_LAYER, "VP");
	}
	...
	public String getTokenNormAsString(){
		return getLayerAsString(NORM_LAYER);
	}
	/**
	 * @return the tokens of this sentence joined by spaces
	 */
	...
	/**
	 * @param i
	 * @return the normalized tag at index i
	 */
	public String getTokenNorm(int i){
		return get(NORM_LAYER,i);
	}
	public void setNormalLayer() {
		Morpha lexer = new Morpha(new ByteArrayInputStream("".getBytes()));
		List<String> morpha = new ArrayList<String>();
		for (int i = 0; i < this.getLength(); i++) {
			String token = this.getToken(i);
			String posTag = this.getPosTag(i);
			...
			morpha.add(tokenNorm);
			//System.out.println(token + " " + posTag + " " + chunkTag + " " + tokenNorm);
		}
		ImmutableList<String>morpha_im = ImmutableList.copyOf(morpha);
		addLayer(NORM_LAYER, morpha_im);
	}
}
