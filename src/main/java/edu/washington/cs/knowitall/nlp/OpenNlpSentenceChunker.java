...
public class OpenNlpSentenceChunker implements SentenceChunker {
	...
	public ChunkedSentence chunkSentence(String[] tokens, String[] posTags) throws ChunkerException {
		ArrayList<Range> ranges = new ArrayList<Range>();
		String[] npChunkTags;
		try {
			int s = 0;
			for (int i = 0; i < tokens.length; i++) {
				Range r = new Range(s, s + tokens[i].length());
				ranges.add(r);
				s = s + tokens[i].length() + 1;
			}
			npChunkTags = chunker.chunk(tokens, posTags);
		} catch (NullPointerException e) {
			throw new ChunkerException("OpenNLP threw NPE on '" + "'", e);
		}
		...
		ChunkedSentence result = new ChunkedSentence(ranges.toArray(new Range[] {}), tokens, posTags, npChunkTags);
		result.setNormalLayer();
		return result;
	}
	...
	public ChunkedSentence chunkSentence(String sent, boolean morpha) throws ChunkerException {
