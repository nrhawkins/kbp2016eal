...
public class LogisticRegression extends Model {
      ...
      try {
        ...
        for (int i = 0; i < labelFactory.allLabels.size(); i++) {
          for (int j = 0; j < featureFactory.allFeatures.size(); j++) {
            writer.println(labelFactory.allLabels.get(i) + ":" + featureFactory.allFeatures.get(j)
  ...
  public void readModel(String file) {
    ...
    //System.out.println("WNEA LR: MODEL LOCATION: " + file);
    //System.out.println("MODEL LOCATION: " + this.getClass().getResource(file).getPath());

    LogisticRegression lr = (LogisticRegression) Serializer.deserialize2(file);
