...
public class ReVerbExtractorWrap {
	...
	// ReVerbIndependentConfFunction confFunc;
	...
	public ReVerbExtractorWrap() {
		try {
			...
			// confFunc = new ReVerbIndependentConfFunction();
		} catch (IOException e) {
...
//	public ReVerbResult parse_align(String tokens_str, String posTags_str) {
//		ReVerbResult rvr = new ReVerbResult();
//		rvr.raw_sentence = tokens_str;
//		String[] tokens = tokens_str.split(" ");
//		String[] posTags = posTags_str.split(" ");
...
//		Map<Integer, Integer> pos2tokenId = new HashMap<Integer, Integer>();
//		int start = 0;
...
//			start = end;
//		}
//		rvr.chunk_sent = chunker.chunkSentence(sb.toString());
//		rvr.chunk_sent.
//		// rvr.chunk_sent = chunker.chunkSentence(tokens, posTags);
//		rvr.reverb_extract = new ArrayList<ChunkedBinaryExtraction>();
//		for (ChunkedBinaryExtraction extr : reverb.extract(rvr.chunk_sent)) {
...
//	}
	...
	private static void writeMyChunk(ChunkedSentence sent, int start, int end, String sentenceid, String chunktype,
			DW dw) {
		if (start == 16) {
			// D.p(start);
		}
		if (start != -1) {
			...
			dw.write(sentenceid, r.getStart(), r.getEnd(), chunktype, sub.getTokensAsString());
		}
	}
		...
		while ((l = dr.read()) != null) {
			...
			ChunkedSentence sent = rvr.chunk_sent;
			{
				...
				for (int i = 0; i < rvr.chunk_sent.getLength(); i++) {
					...
					sb.append(token + "|" + posTag + "|" + chunkTag + "|" + tokenNorm + " ");
				}
			...
			}
			{
				...
				for (int i = 0; i < rvr.chunk_sent.getLength(); i++) {
					...
					if (chunkTag.startsWith("B-")) {
						writeMyChunk(sent, last, i, sentenceid, chunktype, dw_rvphrase);
						last = i;
				...
				if (last > 0) {
					writeMyChunk(sent, last, rvr.chunk_sent.getLength(), sentenceid, chunktype, dw_rvphrase);
				}
			}
			// {
			// // #####NP chunks
			...
			// }
			{
				...
				for (ChunkedBinaryExtraction extr : rvr.reverb_extract) {
					...
					dw_rvrel.write(sentenceid, extr.getArgument1().getRange().getStart(),
							extr.getArgument1().getRange().getEnd(), extr.getRelation().getRange().getStart(),
							extr.getRelation().getRange().getEnd(), extr.getArgument2().getRange().getStart(),
							extr.getArgument2().getRange().getEnd(), extr.getArgument1(), extr.getRelation(),
							extr.getArgument2(), headnoun_extractor.normalizeField(ne.getArgument1()),
							ne.getRelationNorm(), headnoun_extractor.normalizeField(ne.getArgument2()));
	...
	public static void parseText(String input_tokens, String input_pos, String output_prefix) {
		ReVerbExtractorWrap rvew = new ReVerbExtractorWrap();
		...
		while ((l = dr.read()) != null) {
			...
			while (lpos != null && lpos.length > 1 && Integer.parseInt(lpos[0]) < sentenceId) {
				lpos = drpos.read();
			}
			if (lpos != null && lpos.length > 1 && Integer.parseInt(lpos[0]) == sentenceId) {
	...
	public static void main(String[] args) throws Exception {
		...
		String sentStr = "Tom goes to school. ";
		// String sentStr =
		// "Dan was born in USA.";
		// String sentStr =
		...
		ChunkedSentence sent = chunker.chunkSentence(sentStr);
		{
			...
			for (Range r : sent.getNpChunkRanges()) {
				ChunkedSentence sub = sent.getSubSequence(r);
				D.p(r, sub.toString(), sub.getTokenNormAsString(), sub.getPosTagsAsString());
			}
			D.p("#####VP chunks");
			for (Range r : sent.getVpChunkRanges()) {
				ChunkedSentence sub = sent.getSubSequence(r);
		...
		for (int i = 0; i < sent.getLength(); i++) {
			...
			System.out.println(token + " " + posTag + " " + chunkTag + " " + tokenNorm);
		}
		...
		for (ChunkedBinaryExtraction extr : reverb.extract(sent)) {
			...
			System.out.println("Arg1=" + ne.getArgument1Norm() + "\t" + he.normalizeField(ne.getArgument1()));
			System.out.println("Rel=" + ne.getRelationNorm());
			System.out.println("Arg2=" + ne.getArgument2Norm() + "\t" + he.normalizeField(ne.getArgument2()));
